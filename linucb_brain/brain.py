...
class Brain:
    ...
    def recommend(self, student_id: str, topic: Optional[str] = None, top_n: int = 1) -> Union[Content, List[Content]]:
        ...
        if self.model_type == "hybrid" and top_n == 1:
            ...
            # Use full context for clustering
    ...
    def update(self, student_id: Optional[str] = None, content_id: Optional[str] = None, reward: float = 0.0, agent_id: Optional[str] = None, arm_id: Optional[str] = None) -> None:
        ...
        # Build context used for the prediction
        if self.model_type == "disjoint" or self.model_type == "ts":
            ...
            # Update model with cluster_id
            self.model.update(c_id, z_shared, x_arm, reward, cluster_id=cluster_id)
            
            # Track arm recommendation count for exploration efficiency (hybrid only)
            ...
            # Periodically update clustering centroids
            if self.update_count % 10000 == 0:
                # Collect recent contexts for clustering update
    ...
    def predict_grade(self, student_id: str, subject: str) -> float:
        ...
        # We define "Grade" as a baseline performance + historical improvements (rewards)
        # For simplicity, we can use the student's internal performance score, 
        # but normalize it to a 0-100 scale if needed.
        # Let's just return the current performance score as the "predicted next grade".
        # However, to satisfy the requirement of using "weighted average of reward history":
        
        student_sessions = [s for s in self.sessions if s.student_id == student_id and s.topic == subject]
        
        # If we have sessions, the "grade" is the starting grade + sum of rewards
        # Let's assume an initial grade of 0.5 if no history exists.
        initial_grade = 0.5
        ...
        # Cumulative rewards as improvement
    ...
    def save(self, filepath: str) -> None:
        ...
        # Ensure inverses are updated if the model is hybrid
    ...
    def tune_parameters(self) -> Dict[str, float]:
        ...
        # 1. If convergence is low, increase gamma (remember more)
        if scores['convergence_score'] < 5.0:
            self.gamma = min(1.0, self.gamma + 0.01)
        # 2. If exploration is low, increase alpha
        if scores['exploration_score'] < 7.0:
            self.alpha = min(2.0, self.alpha + 0.1)
        # 3. If precision is high but convergence is slow, decrease alpha (exploit more)
